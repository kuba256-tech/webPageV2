...
const Projects = () => {
  ...
  return (
    <section id="projects" className="px-4 mb-[10%]">
      ...
      <div className="flex">
        <div className="flex mx-auto gap-10 bg-gradient-to-r from-fuchsia-600 to-purple-600 rounded-full">
          {ProjectsCategories.map((category) => (
            <div className="relative" key={category}>
              <button
                onClick={() => setSelectedProjects(category)}
                className={`text-xs md:text-3xl px-3  lg:px-20 py-3 z-10 relative capitalize ${selectedProjects == category && "text-black"} rounded-full transition-all ease-in-out`}
              >
                {category}
              </button>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="m-[5%] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10"
        >
